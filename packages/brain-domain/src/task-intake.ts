...
export function extractFilledSlots(
  text: string
): Partial<Record<TaskIntakeSlot, string>> {
  const normalized = normalize(text);
  ...
  if (
    hasAny(normalized, [
      /(중복|오래된|안 읽은|불필요한|중요한 건 빼고|임시 파일|관련 없는|사진만|문서만|탭만|메일만)/,
...
export function buildTaskIntakeSession(
  ...
  const requiredSlots = inferRequiredSlots(sourceText);
  const filledSlots = extractFilledSlots(sourceText);
...
export function mergeTaskIntakeAnswer(
  ...
  const newlyFilled = extractFilledSlots(answerText);
  const filledSlots = {
    ...session.filledSlots,
    ...newlyFilled
  };
